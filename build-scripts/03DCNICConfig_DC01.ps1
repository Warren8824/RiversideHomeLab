...
Write-Host "`n Detecting network adapters..." -ForegroundColor Cyan
    ...
    foreach ($key in $nicRenameMap.Keys) {
        if ($adapter.InterfaceDescription -like "*$key*" -or $adapter.Name -like "*$key*") {
            ...
            if ($adapter.Name -ne $targetName) {
                ...
                Write-Host "[OK] '$targetName' already correctly named"
            }
        }
    }
}
...
Write-Host "`n🎉 Networking configuration complete." -ForegroundColor Cyan
